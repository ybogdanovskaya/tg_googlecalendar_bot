...
class GoogleCalendar:
    ...
    def _create_event_sync(self, request: MeetingRequest) -> str:
        ...
        body: dict[str, Any] = {
            ...
            "start": {"date": request.start_at.date().isoformat()} if request.all_day else {"dateTime": request.start_at.astimezone(UTC).isoformat()},
            "end": {"date": request.end_at.date().isoformat()} if request.all_day else {"dateTime": request.end_at.astimezone(UTC).isoformat()},
