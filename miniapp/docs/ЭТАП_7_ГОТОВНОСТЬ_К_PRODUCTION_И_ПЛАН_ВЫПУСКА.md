...
- 2026-08-09: P2.8 завершён: после approval владельца Mini App выпущена в `main`, production API/статика/Nginx и кнопка BotFather включены, а Telegram UAT подтверждён. Первые две попытки deploy безопасно остановились до изменения production из-за прав staged release; причина устранена отдельным PR и обновлением restricted server-side entrypoint. Подробности — в [этапе 8.8](ЭТАП_8_8_PRODUCTION_ВЫПУСК_И_UAT.md).
