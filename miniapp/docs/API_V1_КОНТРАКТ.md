...
На локальном этапе 2 реализованы: Telegram auth с серверной проверкой `initData`, короткая cookie-сессия, CSRF, `GET /me`, согласие с политикой, конфигурация бронирования, календарные даты и слоты без раскрытия содержимого Google Calendar, а также список, просмотр, создание, изменение и отмена pending-заявки. Создание защищено серверной идемпотентностью.

Пользовательские альтернативы, запросы переноса/отмены назначенной встречи и удаление данных будут реализованы и протестированы вместе с пользовательским frontend на этапе 3. Административные маршруты — вместе с административным интерфейсом на этапе 4. До этого таблицы ниже описывают целевой, а не уже доступный UI-контракт.
