...
- 2026-08-09: UAT правил записи завершена: приостановка новых заявок показывает пользователю понятное сообщение и блокирует мастер записи; после повторного включения мастер снова показывает доступные длительности. Test-контур возвращён в режим приёма заявок.
- 2026-08-09: UAT закрытых дат завершена: администратор закрыл тестовую дату, она исчезла из пользовательского календаря, затем дата была удалена из списка закрытых; исключение очищено из test-базы и дата снова доступна.
- 2026-08-09: UAT расширенной настройки завершена: значение минимального времени до встречи изменено с 120 до 180 минут, сохранение подтверждено preview-базой; после проверки возвращено исходное значение 120 минут.
- 2026-08-09: UAT автоматических напоминаний завершена: переключатель приостановил автоматизацию, состояние подтверждено preview-базой; после проверки автоматические напоминания снова включены.
- 2026-08-09: финальная проверка preview завершена: локально успешно пройдены 92 Python-теста, 6 frontend-тестов, production-сборка Mini App и проверка секретов; preview и рабочий бот активны, HTTPS и health-check preview отвечают успешно, в preview-журнале нет записей уровня `error`. Последний CI ветки `dev` завершён успешно на актуальном коммите.
