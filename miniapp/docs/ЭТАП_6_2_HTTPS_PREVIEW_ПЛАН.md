...
Статус: подготовка внешних тестовых ресурсов выполнена; серверное preview-развёртывание ещё не начиналось. Этот документ не является разрешением на изменение DNS, BotFather, VPS, Nginx, systemd, GitHub secrets или production.

## Журнал выполнения

- 2026-08-09: создан отдельный тестовый Telegram-бот; его токен сохранён только в локальном игнорируемом `.env`.
- 2026-08-09: создан отдельный пустой тестовый Google Calendar; его ID сохранён только в локальном игнорируемом `.env`.
- 2026-08-09: создана копия Apps Script, перенастроенная на тестовый календарь, с отдельным секретом и отдельным Web App URL.
- 2026-08-09: выполнена безопасная проверка опубликованного тестового gateway: `POST busy` вернул HTTP 200 и корректный пустой список занятости. Встречи не создавались и production-ресурсы не использовались.
- 2026-08-09: создана DNS A-запись `calendar-dev`; авторитетный DNS-сервер подтвердил, что она указывает на выделенный VPS preview. Распространение записи во внешних DNS может занять до нескольких часов.

Не записываются в этот журнал: токены, календарные ID, URL Apps Script, секреты и персональные данные.
