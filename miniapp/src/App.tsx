...
function Home({ name, onBook, onRequests }: { name: string; onBook: () => void; onRequests: () => void }) {
  return <><section className="hero"><p className="eyebrow">Добро пожаловать{name ? `, ${name}` : ""}</p><h1>Выберите удобное время для встречи</h1><button className="button primary" onClick={onBook}>Записаться на встречу</button></section><section className="info-card"><span>Мои встречи</span><p>Проверьте статус заявки, измените или отмените её, пока резерв активен.</p><button className="text-button" onClick={onRequests}>Открыть список →</button></section></>;
}
...
function Booking({ api, onDone, onBack }: { api: CalendarApi; onDone: () => void; onBack: () => void }) {
  ...
  useEffect(() => { void api.bookingConfig().then((value) => { setConfig(value); setDraft((current) => ({ ...current, durationMinutes: current.durationMinutes ?? (value.durations.includes(30) ? 30 : value.durations[0] ?? null) })); }).catch((caught) => setError(friendlyError(caught))).finally(() => setLoading(false)); }, [api]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setLoading(true); setError("");
    void api.bookingSlots(draft.date, draft.durationMinutes).then((response) => { setSlots(response.slots); setPeriodCounts(response.period_counts); }).catch((caught) => setError(friendlyError(caught))).finally(() => setLoading(false));
  ...
  const submit = async () => {
    ...
    try {
      ...
      onDone();
    } catch (caught) { setError(friendlyError(caught)); if (caught instanceof ApiError && caught.code === "SLOT_UNAVAILABLE") setStep(4); } finally { setSubmitting(false); }
  };
  ...
  const titles = ["Выберите длительность", "Выберите дату", "Выберите период дня", "Выберите время", "Расскажите о встрече", "Проверьте заявку"];
  const periods = [{ key: "morning", title: "Утро", range: "до 12:00" }, { key: "day", title: "День", range: "12:00–17:00" }, { key: "evening", title: "Вечер", range: "после 17:00" }];
  const filteredSlots = period ? slots.filter((slot) => { const hour = Number(slot.slice(11, 13)); return period === "morning" ? hour < 12 : period === "day" ? hour < 17 : hour >= 17; }) : [];
  const goBack = () => { if (step === 1) onBack(); else { if (step === 4) setPeriod(null); setStep(step - 1); } };
  return <section className="flow"><div className="flow-heading"><button className="back" onClick={goBack}>←</button><div><span className="eyebrow">Запись · шаг {step} из 6</span><h1>{titles[step - 1]}</h1></div></div><div className="progress"><i style={{ width: `${step / 6 * 100}%` }} /></div>{error && <p className="error" role="alert">{error}</p>}
    {step === 1 && <><div className="choice-grid">{config.durations.map((duration) => <button className={`choice ${draft.durationMinutes === duration ? "selected" : ""}`} key={duration} onClick={() => setDraft({ ...draft, durationMinutes: duration, date: null, slot: null })}>{duration} минут</button>)}</div><button className="button primary flow-next" disabled={!draft.durationMinutes} onClick={() => setStep(2)}>Продолжить</button></>}
    {step === 2 && <MonthCalendar dates={dates} selected={draft.date} offset={monthOffset} onPrevious={() => setMonthOffset(Math.max(0, monthOffset - 1))} onNext={() => setMonthOffset(Math.min(2, monthOffset + 1))} onSelect={(date) => { setDraft({ ...draft, date, slot: null }); setPeriod(null); setStep(3); }} />}
    {step === 3 && <>{loading ? <SectionLoading label="Проверяем свободное время…" /> : <div className="choice-grid period-grid">{periods.map((item) => <button className="choice period-choice" key={item.key} disabled={!periodCounts[item.key]} onClick={() => { setPeriod(item.key); setStep(4); }}><strong>{item.title}</strong><span>{periodCounts[item.key] ? `${periodCounts[item.key]} свободных` : "Нет времени"}</span><small>{item.range}</small></button>)}</div>}</>}
    {step === 4 && <>{loading ? <SectionLoading label="Проверяем свободное время…" /> : filteredSlots.length ? <div className="slot-grid">{filteredSlots.map((slot) => <button key={slot} className={`slot-button ${draft.slot === slot ? "selected" : ""}`} onClick={() => { setDraft({ ...draft, slot }); setStep(5); }}>{timeOf(slot)}</button>)}</div> : <Empty label="В выбранный период нет свободного времени." />}</>}
    {step === 5 && <MeetingForm draft={draft} onChange={setDraft} onNext={() => setStep(6)} />}
    {step === 6 && <div className="review-card"><p className="status-line">На согласовании</p><h2>{draft.subject}</h2><p>{draft.slot && localizedDate(draft.slot, true)} · {draft.durationMinutes} минут</p><p>{draft.name} · {draft.email}</p>{draft.location && <p>{draft.location}</p>}<p className="muted">Слот будет зарезервирован на срок, установленный правилами записи.</p><button className="button primary" disabled={submitting} onClick={() => void submit()}>{submitting ? "Отправляем…" : "Отправить заявку"}</button></div>}
  </section>;
}

function MonthCalendar({ dates, selected, offset, onPrevious, onNext, onSelect }: { dates: string[]; selected: string | null; offset: number; onPrevious: () => void; onNext: () => void; onSelect: (date: string) => void }) {
  ...
  return <div className="month-calendar"><div className="month-nav"><button className="back" aria-label="Предыдущий месяц" disabled={offset === 0} onClick={onPrevious}>‹</button><strong>{monthLabel}</strong><button className="back" aria-label="Следующий месяц" disabled={offset === 2} onClick={onNext}>›</button></div><div className="weekday-row">{["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"].map((item) => <span key={item}>{item}</span>)}</div><div className="month-grid">{Array.from({ length: firstWeekday }, (_, index) => <span key={`empty-${index}`} />)}{Array.from({ length: days }, (_, index) => { const date = isoDate(new Date(year, monthIndex, index + 1)); const enabled = available.has(date); return <button key={date} disabled={!enabled} className={selected === date ? "selected" : ""} onClick={() => onSelect(date)}>{index + 1}</button>; })}</div></div>;
}

function MeetingForm({ draft, onChange, onNext }: { draft: RequestDraft; onChange: (value: RequestDraft) => void; onNext: () => void }) {
  const valid = draft.name.trim() && !draft.name.includes("@") && /^\S+@\S+\.\S+$/.test(draft.email) && draft.subject.trim();
  const update = (field: keyof RequestDraft, value: string) => onChange({ ...draft, [field]: value });
  return <form className="form" onSubmit={(event) => { event.preventDefault(); if (valid) onNext(); }}><label>Имя<input name="requester_name" autoComplete="name" value={draft.name} maxLength={120} onChange={(event) => update("name", event.target.value)} required /></label><label>Email<input name="email" autoComplete="email" inputMode="email" value={draft.email} type="email" maxLength={254} onChange={(event) => update("email", event.target.value)} required /></label><label>Тема встречи<input value={draft.subject} maxLength={200} onChange={(event) => update("subject", event.target.value)} required /></label><label>Описание <span>необязательно</span><textarea value={draft.description} maxLength={4000} onChange={(event) => update("description", event.target.value)} /></label><label>Место или ссылка <span>необязательно</span><input value={draft.location} maxLength={1000} onChange={(event) => update("location", event.target.value)} /></label><button className="button primary" disabled={!valid}>Продолжить</button></form>;
}
...
function Requests({ api, onBook }: { api: CalendarApi; onBook: () => void }) {
  ...
  const load = () => { setLoading(true); setError(""); void api.requests().then((response) => { setItems(response.items ?? []); setArchive(response.archive ?? []); }).catch((caught) => setError(friendlyError(caught))).finally(() => setLoading(false)); };
  useEffect(load, [api]);
