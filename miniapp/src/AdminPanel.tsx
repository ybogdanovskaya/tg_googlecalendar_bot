...
export function AdminPanel({ api }: { api: CalendarApi }) {
  ...
  const [allDaySubject, setAllDaySubject] = useState("");
  const [allDayStart, setAllDayStart] = useState("");
  const [allDayEnd, setAllDayEnd] = useState("");
  const [allDayDescription, setAllDayDescription] = useState("");
  const [allDayLocation, setAllDayLocation] = useState("");
  ...
  const load = () => {
    ...
    void Promise.all([api.adminDashboard(), api.adminRequests(), api.adminSettings(), api.adminClosedDates(), api.adminSeries(), api.adminChangeRequests(), api.adminManualMeetings(), api.adminStatistics(), api.adminCalendarIntegration()])
      .then(([nextDashboard, nextRequests, nextSettings, nextClosedDates, nextSeries, nextChanges, nextManualMeetings, nextStatistics, nextIntegration]) => {
        setDashboard(nextDashboard); setRequests(nextRequests); setSettings(nextSettings); setClosedDates(nextClosedDates.items ?? []); setClosedWeekdays(nextClosedDates.weekdays ?? []); setSeries(nextSeries); setChanges(nextChanges); setManualMeetings(nextManualMeetings); setStatistics(nextStatistics); setIntegration(nextIntegration);
      })
  ...
  return <section className="admin-panel">
    ...
    <article className="info-card"><div className="section-heading compact"><div><span className="eyebrow">Заявки</span><h2>Ожидают согласования</h2></div><span className="role-badge">{requests.length}</span></div>
      {requests.length === 0 ? <p className="muted">Сейчас нет заявок, ожидающих решения.</p> : <div className="request-list">{requests.map((request) => <article className="request-card admin-request" key={request.id}>
        ...
        {editingRequest?.id === request.id && <MeetingEditor api={api} item={editingRequest} onDone={() => { setEditingRequest(null); load(); }} onCancel={() => setEditingRequest(null)} />}
    ...
    <article className="info-card"><span className="eyebrow">Ручное создание</span><h2>Добавить встречу</h2><form className="form compact-form" onSubmit={(event) => { event.preventDefault(); if (!manualSubject || !manualStart) return; void act("manual-meeting", async () => { await api.createAdminManualMeeting({ subject: manualSubject, email: manualEmail || undefined, start_at: new Date(manualStart).toISOString(), duration_minutes: manualDuration, blocks_calendar: manualBlocksCalendar, allow_overlap: false }); setManualSubject(""); setManualEmail(""); setManualStart(""); setManualBlocksCalendar(true); }); }}><label>Тема<input value={manualSubject} maxLength={200} onChange={(event) => setManualSubject(event.target.value)} required /></label><label>Email гостя <span>необязательно</span><input type="email" value={manualEmail} onChange={(event) => setManualEmail(event.target.value)} /></label><div className="form-row"><label>Дата и время<input type="datetime-local" value={manualStart} onChange={(event) => setManualStart(event.target.value)} required /></label><label>Минуты<select value={manualDuration} onChange={(event) => setManualDuration(Number(event.target.value))}>{[15, 30, 45, 60, 90, 120].map((value) => <option key={value}>{value}</option>)}</select></label></div><label className="checkbox-control"><input type="checkbox" checked={manualBlocksCalendar} onChange={(event) => setManualBlocksCalendar(event.target.checked)} />Блокировать это время для записи</label><button className="button primary" disabled={busy !== null}>{busy === "manual-meeting" ? "Создаём…" : "Создать встречу"}</button></form>{editingManualMeeting && <MeetingEditor api={api} item={editingManualMeeting} manual onDone={() => { setEditingManualMeeting(null); load(); }} onCancel={() => setEditingManualMeeting(null)} />}{manualMeetings.length > 0 && <div className="request-list series-list">{manualMeetings.map((item) => <article className="request-card" key={item.id}><div className="request-meta"><span>{item.status_label}</span>{item.all_day && <span>На весь день</span>}{!item.all_day && !item.blocks_calendar && <span>Не блокирует запись</span>}</div><h2>{item.subject}</h2><p>{item.all_day ? dateRange(item.start_at, item.end_at) : `${dateTime(item.start_at)} · ${item.duration_minutes} мин.`}</p>{item.status === "APPROVED" && <div className="admin-actions">{!item.all_day && <button className="text-button" disabled={busy !== null} onClick={() => setEditingManualMeeting(item)}>Редактировать</button>}<button className="text-button danger" disabled={busy !== null} onClick={() => { if (window.confirm(item.all_day ? "Отменить это многодневное событие в календаре?" : "Отменить эту ручную встречу в календаре?")) void act(`manual-${item.id}`, () => api.cancelAdminManualMeeting(item.id)); }}>{item.all_day ? "Отменить событие" : "Отменить встречу"}</button></div>}</article>)}</div>}</article>

    <article className="info-card"><span className="eyebrow">Длительное событие</span><h2>Заблокировать даты</h2><p className="muted">Например, отпуск. Все даты диапазона включительно будут недоступны для записи.</p><form className="form compact-form" onSubmit={(event) => { event.preventDefault(); if (!allDaySubject || !allDayStart || !allDayEnd) return; void act("all-day-event", async () => { await api.createAdminAllDayEvent({ subject: allDaySubject, description: allDayDescription || undefined, location: allDayLocation || undefined, start_date: allDayStart, end_date: allDayEnd }); setAllDaySubject(""); setAllDayStart(""); setAllDayEnd(""); setAllDayDescription(""); setAllDayLocation(""); }); }}><label>Тема<input value={allDaySubject} maxLength={200} onChange={(event) => setAllDaySubject(event.target.value)} required /></label><div className="form-row"><label>Начальная дата<input type="date" value={allDayStart} onChange={(event) => setAllDayStart(event.target.value)} required /></label><label>Конечная дата<input type="date" min={allDayStart || undefined} value={allDayEnd} onChange={(event) => setAllDayEnd(event.target.value)} required /></label></div><label>Описание <span>необязательно</span><textarea value={allDayDescription} maxLength={4000} onChange={(event) => setAllDayDescription(event.target.value)} /></label><label>Место или ссылка <span>необязательно</span><input value={allDayLocation} maxLength={1000} onChange={(event) => setAllDayLocation(event.target.value)} /></label><button className="button primary" disabled={busy !== null}>{busy === "all-day-event" ? "Создаём…" : "Создать блокирующее событие"}</button></form></article>

    <article className="info-card"><span className="eyebrow">Повторяющиеся встречи</span><h2>Новая серия</h2><form className="form compact-form" onSubmit={(event) => { event.preventDefault(); if (!seriesSubject || !seriesStart || !seriesUntil) return; void act("series", async () => { await api.createAdminSeries({ subject: seriesSubject, start_at: new Date(seriesStart).toISOString(), duration_minutes: seriesDuration, frequency: seriesFrequency, until_date: seriesUntil, blocks_calendar: seriesBlocksCalendar, allow_overlap: false }); setSeriesSubject(""); setSeriesStart(""); setSeriesUntil(""); setSeriesBlocksCalendar(true); }); }}><label>Тема<input value={seriesSubject} maxLength={200} onChange={(event) => setSeriesSubject(event.target.value)} required /></label><div className="form-row"><label>Первая встреча<input type="datetime-local" value={seriesStart} onChange={(event) => setSeriesStart(event.target.value)} required /></label><label>Минуты<select value={seriesDuration} onChange={(event) => setSeriesDuration(Number(event.target.value))}>{[15, 30, 45, 60, 90, 120].map((value) => <option key={value}>{value}</option>)}</select></label></div><div className="form-row"><label>Повтор<select value={seriesFrequency} onChange={(event) => setSeriesFrequency(event.target.value as EventSeries["frequency"])}><option value="DAILY">Ежедневно</option><option value="WEEKLY">Еженедельно</option><option value="MONTHLY">Ежемесячно</option></select></label><label>До даты<input type="date" value={seriesUntil} onChange={(event) => setSeriesUntil(event.target.value)} required /></label></div><label className="checkbox-control"><input type="checkbox" checked={seriesBlocksCalendar} onChange={(event) => setSeriesBlocksCalendar(event.target.checked)} />Блокировать время повторений для записи</label><button className="button primary" disabled={busy !== null}>{busy === "series" ? "Создаём…" : "Создать серию"}</button></form>
    ...
    <article className="info-card"><span className="eyebrow">Правила записи</span><h2>Доступность для пользователей</h2>
      ...
      <form className="form compact-form" onSubmit={(event) => { event.preventDefault(); const values = new FormData(event.currentTarget); const start = minutesFromClock(values.get("booking_start")); const end = minutesFromClock(values.get("booking_end")); if (start === null || end === null || start >= end) { setError("Начало окна должно быть раньше окончания."); return; } void act("booking-window", () => api.updateAdminSetting("user_booking_window", [start, end])); }}><div className="form-row"><label>Начало окна<input name="booking_start" type="time" step="900" defaultValue={clockFromMinutes(settings.booking.user_booking_window[0])} required /></label><label>Конец окна<input name="booking_end" type="time" step="900" defaultValue={clockFromMinutes(settings.booking.user_booking_window[1])} required /></label></div><button className="text-button" disabled={busy !== null}>{busy === "booking-window" ? "Сохраняем…" : "Сохранить время записи"}</button></form><p className="muted">Доступные длительности: {settings.booking.durations.join(", ")} мин.</p></>}
    </article>
    ...
    <article className="info-card"><span className="eyebrow">Нерабочие дни</span><h2>Закрытые даты и выходные</h2><div className="weekday-controls">{[{ value: 5, label: "Каждую субботу" }, { value: 6, label: "Каждое воскресенье" }].map((day) => <label key={day.value}><input type="checkbox" checked={closedWeekdays.includes(day.value)} disabled={busy !== null} onChange={() => void act("closed-weekdays", () => api.updateAdminSetting("closed_weekdays", closedWeekdays.includes(day.value) ? closedWeekdays.filter((value) => value !== day.value) : [...closedWeekdays, day.value]))} />{day.label}</label>)}</div><form className="inline-form" onSubmit={(event) => { event.preventDefault(); if (closedDate) void act("closed-date", async () => { await api.addAdminClosedDate(closedDate); setClosedDate(""); }); }}><input type="date" value={closedDate} onChange={(event) => setClosedDate(event.target.value)} required /><button className="text-button" disabled={busy !== null}>Добавить</button></form>
